...
"$ROOT_DIR/backend/mvnw" -f "$ROOT_DIR/backend/pom.xml" test -Dgroups=unit "$@"
...
if [[ "$test_count" -eq 0 ]]; then
  echo 'Unit test suite executed zero backend tests.' >&2
  exit 1
fi

printf 'Backend unit tests executed: %s\n' "$test_count"
