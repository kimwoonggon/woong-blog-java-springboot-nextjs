...
test('admin can run a small Work and Study read load test', async ({ page, request }, testInfo) => {
  ...
  await expect(page.getByTestId('load-test-runtime-panel').getByText('Backend runtime', { exact: true })).toBeVisible()
  ...
  await expect(page.getByTestId('load-test-runtime-panel')).toContainText(/Runtime workers/)
  await expect(page.getByTestId('load-test-runtime-panel')).toContainText(/Runtime queue/)
  ...
  await expect(page.getByTestId('load-test-database-panel')).toContainText(/JDBC pool size/)
  await expect(page.getByTestId('load-test-database-panel')).toContainText(/JDBC max pool/)
...
test('admin can run and stop a real backend test with polling + metrics fallback', async ({ page, request }, testInfo) => {
  ...
  await expect(panel.getByText(/Application elapsed p95: 35\.4 ms/i)).toBeVisible()
  ...
  await expect(page.getByTestId('load-test-runtime-panel')).toContainText(/samples collected from the backend runtime/)
  await expect(page.getByTestId('load-test-database-panel')).toContainText(/DB command P95/)
  await expect(page.getByTestId('load-test-database-panel')).toContainText(/JDBC max pool/)
